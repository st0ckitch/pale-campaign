...
const SUBJECTS = ['Mathematics', 'Combined Science', 'English Language']
...
export default function Teacher({ t, store, toast, reduceMotion, onGo }) {
  ...
  const selCount = Object.values(selected).filter(Boolean).length + customQs.length
  ...
  function addCustomQuestion() {
    const opts = cOpts.map((o) => o.trim())
    if (!cPrompt.trim() || opts.some((o) => !o)) {
      toast('Fill the question and all four options first')
      return
    }
    setCustomQs((qs) => [
      ...qs,
      {
        id: rid('tc'),
        type: 'mcq',
        topic: cTopic.trim() || subject,
        prompt: cPrompt.trim(),
        latex: '',
        options: opts,
        correctAnswer: opts[cCorrect],
        acceptedAnswers: [],
        workingNotes: 'Set by your teacher.',
      },
    ])
    setCPrompt(''); setCTopic(''); setCOpts(['', '', '', '']); setCCorrect(0)
    toast('Question added to this exam')
  }

  function createExam() {
    if (!title.trim()) { toast('Give the exam a title'); return }
    const questionIds = QUESTION_BANK.filter((q) => selected[q.id]).map((q) => q.id)
    if (questionIds.length + customQs.length === 0) { toast('Add at least one question'); return }
    store.addExam({ title: title.trim(), subject, durationMin: Number(durationMin) || 20, due: due.trim() || 'Anytime', questionIds, customQuestions: customQs })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, animation: reduceMotion ? 'none' : `qgfade .4s ${t.EASE} both` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12.5, color: sub(0.5) }}>Publish exams and announcements — they appear instantly on the student side.</div>
        </div>
      </div>
      ...
      <section style={{ ...t.GLASS, borderRadius: 24, padding: '26px 28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 14 }}>
          <div><label style={labelStyle}>Title</label><input style={inputStyle} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Year 11 Algebra Checkpoint" /></div>
          <div><label style={labelStyle}>Subject</label>
            <select style={{ ...inputStyle, appearance: 'auto' }} value={subject} onChange={(e) => setSubject(e.target.value)}>
        ...
        <div style={{ marginTop: 18 }}>
          <label style={labelStyle}>Include questions from the bank ({selCount} selected)</label>
        ...
        <div style={{ marginTop: 18, padding: 16, borderRadius: 16, background: fill(0.03), border: `1px solid ${fill(0.08)}` }}>
          <label style={labelStyle}>…or write your own multiple-choice question</label>
          <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: 10 }}>
            <input style={inputStyle} value={cPrompt} onChange={(e) => setCPrompt(e.target.value)} placeholder="Question prompt" />
            <input style={inputStyle} value={cTopic} onChange={(e) => setCTopic(e.target.value)} placeholder="Topic" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
            {cOpts.map((o, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button onClick={() => setCCorrect(i)} title="Mark correct" style={{ width: 22, height: 22, flexShrink: 0, borderRadius: 999, cursor: 'pointer', border: `1px solid ${cCorrect === i ? t.OK : fill(0.3)}`, background: cCorrect === i ? t.OK : 'transparent', color: '#0B0D10', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11 }}>{cCorrect === i ? '✓' : ''}</button>
                <input style={inputStyle} value={o} onChange={(e) => setCOpts((arr) => arr.map((x, j) => (j === i ? e.target.value : x)))} placeholder={`Option ${String.fromCharCode(65 + i)}${cCorrect === i ? ' (correct)' : ''}`} />
              </div>
            ))}
          </div>
          <button style={{ ...t.ghostBtn, marginTop: 12 }} onClick={addCustomQuestion}>+ Add this question</button>
          {customQs.length > 0 && (
            <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {customQs.map((q, i) => (
                <div key={q.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: sub(0.75) }}>
                  <span style={{ color: t.OK }}>✓</span> {i + 1}. {q.prompt}
        ...
        <div style={{ display: 'flex', gap: 14, alignItems: 'center', marginTop: 18, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: 12.5, color: sub(0.5) }}>{selCount} question{selCount === 1 ? '' : 's'} in this exam</span>
        </div>
      </section>
