...
export async function analyzeErrors(attempts, { examTitle = null, signal } = {}) {
  ...
  const user =
    'Lost-mark records from recent class exams (one JSON object per line):\n' +
    mistakes.map((m) => JSON.stringify(m)).join('\n') +
    ...
    '"count": number of lost-mark records in this cluster, ' +
    '"students": [distinct student names affected], ' +
    '"examples": [1-2 short quotes of actual wrong answers with their question], ' +
    '"reteach": [2-3 short concrete reteaching actions, e.g. a 10-min starter, a targeted worked example], ' +
    '"severity": "high" | "medium" | "low" } ] }\n' +
    'Order clusters by severity then count.'
  ...
  return {
    ...
    clusters: (Array.isArray(obj.clusters) ? obj.clusters : []).map((c) => ({
      ...
      students: Array.isArray(c.students) ? c.students.map(String) : [],
